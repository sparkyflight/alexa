...
const getAndValidatePortNumber = () => {
    const portNumberArgument = Number(getArgument('portNumber', defaultPort));

    if (!Number.isInteger(portNumberArgument)) throw new Error(`Port number has to be an integer - ${portNumberArgument}.`);
    if (portNumberArgument < 0 || portNumberArgument > 65535) throw new Error(`Port out of legal range: ${portNumberArgument}. The port number should be in the range [0, 65535]`);
    if (portNumberArgument === 0) console.log('The TCP server will listen on a port that is free.'
        + 'Check logs to find out what port number is being used');
    
    return portNumberArgument;
}

const getLambdaHandlerName = () => { return getArgument('lambdaHandler', defaultHandlerName); };

const getAndValidateSkillInvokerFile = () => {
    const fileNameArgument = getArgument('skillEntryFile');
    
    if (!existsSync(fileNameArgument)) throw new Error(`File not found: ${fileNameArgument}`);
    return fileNameArgument;
}

const getArgument = (argumentName, defaultValue) => {
    const index = process.argv.indexOf(`--${argumentName}`);

    if (index === -1 || typeof process.argv[index + 1] === 'undefined') {
        if (defaultValue === undefined) throw new Error(`Required argument - ${argumentName} not provided.`);
